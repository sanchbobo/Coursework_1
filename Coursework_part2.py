def get_outcomes(pass_input_value, defer_input_value, fail_input_value: int) -> str:
...
def get_multiple_students_outcomes():
    ...
    y_or_q = 'y'
    while y_or_q == 'y':
        outcome, lst_values = get_single_student_outcome()
...
def get_multiple_students_outcomes_pt2():
    lst_of_outcomes = [[]]
    y_or_q = 'y'
    while y_or_q == 'y':
        outcome, lst_values = get_single_student_outcome()
        print(outcome)
        tmp_lst = [] 
        tmp_lst.append(outcome)
        tmp_lst.append(lst_values)
        lst_of_outcomes.append(tmp_lst)
        print("Would you like to enter another set of data?\n")
        print("Enter 'y' for yes or 'q' to quit and view results: ")
        while True:
            y_or_q = input()
            if (y_or_q == 'q' or y_or_q == 'y'):
                ...
                print("Wrong query. Try again")
                continue
    for i in range(len(lst_of_outcomes)):
        print(*lst_of_outcomes[i], sep = " - ")


get_multiple_students_outcomes_pt2()